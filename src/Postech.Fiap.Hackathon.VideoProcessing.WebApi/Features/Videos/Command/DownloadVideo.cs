...
public abstract class DownloadVideo
{
           ...
           : IRequestHandler<Command, Result<DownloadVideoZipResponse>>
    {
        public async Task<Result<DownloadVideoZipResponse>> Handle(Command request, CancellationToken cancellationToken)
        {
            ...
            var result = await videoService.download(new DownloadVideoZipRequest(request.Id), cancellationToken);
            ...
            return Result.Success(new DownloadVideoZipResponse(result.Value.File, result.Value.ContentType, result.Value.FileName));
        }
    }
